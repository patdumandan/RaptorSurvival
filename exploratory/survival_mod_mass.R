...
data{
 ...
 model {
  ...
  mass_eff~ normal (0.1,1);//mean informed from Newton paper (mass coef)
  est_eff~ normal (0,1);
  //diet_eff~normal(0,1);
  //for_eff~normal(0,1);
  sp_non~ normal(0,1);
...
print(surv_mod_mass, pars=c("alpha_fam", "alpha", "mass_eff"))
saveRDS(surv_mod_mass, "survival_mass.RDS")
post=rstan::extract(surv_mod_mass)$pred_surv
mean(post)
matplot(surv1$Average.mass..kg.,t(post), type="l", col="grey", xlab="average mass (g)", ylab="survival estimate", ylim=c(0.0,1.0))
