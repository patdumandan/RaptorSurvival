...
//  vector [N] forage; //ground foraging strategy
 ...
 parameters {
  ...
  vector <lower=0, upper=1> [N] pred_surv; //mean estimated survival 
  vector <lower=0> [N] A;
  vector <lower=0> [N] B;
 ...
 // mass_fam[m]=mass_eff+phi*sigma_fam[m];
  ...
  pred_surv[i]= inv_logit(alpha_sp[species[i]]+alpha_st[study[i]]+alpha_fam[family[i]]+
...
", data=list(N=length(surv$Species), y=surv$estimated.survived, n=surv$sample.size,
             ...
             Nfam=6, Nst=65, Nsp=37), chains=4, iter=4000, warmup=1000, control=list(adapt_delta=0.99, max_treedepth=15))

saveRDS(surv1, file="survival_predsurv_lowESS.RDS")
post=rstan::extract(surv1)$pred_y #predicted survival estimate
mean(post)
print(meta_survival_spstfam_fin, pars=c("alpha", "mass_eff","diet_eff", "est_eff","for_eff", "alpha_fam"))
modelfin=rstan::extract(meta_survival_spstfam_fin)
print(surv1, pars=c("alpha_sp"))
